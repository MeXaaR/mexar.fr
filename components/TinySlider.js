...
const TinySlider = ({ children, options = {}, slider, setSlider, listRef }) => {
  ...
  }, [setSlider, slider, innerSlider, listRef, options]);

  return (
    <div ref={listRef} className={`${options.container?.split(".")[1]} position-relative`}>
      {children}
    </div>
  );
};
